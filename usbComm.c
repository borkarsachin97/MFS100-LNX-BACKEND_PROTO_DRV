...
bool isDeviceConnected(libusbAPI *api, USB *dev) 
{
    ...
    libusb_free_device_list(device_list, 1); // Freeing device list
...
int readData(USB *dev, libusbAPI *api)
{
	...
	int rdResult = libusb_bulk_transfer(api->handle, dev->endPointOUT1, dev->data, dev->sizeOfData, &dev->read, dev->readTime);
	
	if (rdResult)
	{
        printf("rdResult: %d, dev->read:%d", rdResult, dev->read);
    }
    else 
    {
...
{
    int transResult = libusb_bulk_transfer(api->handle, dev->endPointIN1, dev->data, dev->sizeOfData, &dev->transferred, dev->transTime);
